...
const generatorArgs = {
  generator: {
    type: String,
    required: false,
    description: `Name of the generator to invoke`,
  },
  subcommand: {
    type: String,
    required: false,
    description: `Name of the subcommand to invoke`,
  },
};
  ...
  async prompting() {
    ...
    if (this.options.generator === "test") {
      generatorPath = path.join(
        __dirname,
        "../plugin-generators/generator-hanacloudui5-test"
      );
    } else {
      ...
      let generator =
        ...
        reqRepos.data.find(
          (repo) => repo.name === `hanacloudui5-app-template`
        );
      ...
      if (!generator) {
        ...
        const generatorRepos = reqRepos.data.filter((repo) =>
          /^generator-hanacloudui5-.+/.test(repo.name)
        );
        const generatorIdx = (
          await this.prompt([
            {
